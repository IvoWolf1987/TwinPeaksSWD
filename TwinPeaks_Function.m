function [swd_number,percent_time_swd,time_grid,final_selection] = TwinPeaks_Function(cx,SR)
...
window = SR*1; %Define the size of the signal segments that will be used for a sliding fft, as a magnitude of sampling rates (results in 0.5 seconds window)
...
[result1, frequencymap1, timepoints1, power1] = spectrogram(cxraw,window,round(SR*0.5),SR*10,SR); %obtain a matrix of amplitudes, the corresponding frequencies, timepoints and power for a sliding fft with 0.5 seconds window-size (using predefined window value, see above), window-overlapp of 0.5 seconds, and zero-padding for 0.1 Hz resolution (10*SR)
...
[r,c] = size(result1); %obtain size parameters of the results for subsequent loop-construction
  ...
  selector1 = (scount >= 2) & (ppks >= 3) & (Pks >= 2) & (artcount == 0); %search criteria, first round
...
pstonex = mz + sz;
  ...
  selector3 = (scounto >= 2) & (ppks >= 3) & (Pks >= 2) & (artcount == 0); % search criteria, second round
